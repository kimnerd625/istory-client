...
const MkCodeButton = ({ userId }: MkCodeButtonProps) => {
  ...
  return (
    <div className="w-full flex flex-row justify-center items-center px-5">
      <button
        onClick={handleInviteCodeGeneration} // 클릭 시 초대 코드 생성 및 처리 함수 실행
        disabled={loading} // 로딩 중일 때 버튼을 비활성화하여 중복 클릭을 방지
        className="w-full rounded-xl py-[18px] flex flex-row justify-center items-center bg-main-400 bg-main-40"
      >
